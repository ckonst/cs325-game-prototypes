...
GameStates.makeGame = function (game, shared) {
    const cs1 = [
        ...
        '\[space\] to shoot',
        'Defeat Chicken Omega as fast as possible.',
        'Avoid salmonella from raw eggs.',
    ];
    let line = [];
    let wordIndex = 0;
    let lineIndex = 0;
    ...
    let text, skipText;
    let bg;
    let music, eHurt, eShoot, pHurt, pShoot;
    let tutFlag = true;
    let Chicken;
    let hover = false;
    let hitBox = null;
    ...
    function eggPop(x, y, i, phase) {
        ...
        egg.angle = game.physics.arcade.moveToXY(
            egg,
            player.x,
            player.y,
            150 * phase,
    ...
    return {
        ...
        update: function () {
            ...
            if (cursors.left.isDown || wasd.left.isDown) {
                if (player.body !== null) player.body.velocity.x = -pSpeed;
            ...
            if (cursors.up.isDown || wasd.up.isDown) {
                if (player.body !== null) player.body.velocity.y = -pSpeed;
                ...
                if (player.body !== null) player.body.velocity.y = 0;
                if (directionY !== 'none') {
                    directionY = 'none';
                }
            }
            ...
            if (shoot_button.isDown && time > projectileTimer) {
                const randy = Math.floor(Math.random() * 5 + 1);
                switch (randy) {
                    case 1:
                        knife = game.add.sprite(
                            player.x,
                            player.y - 45,
                            'cutlery1',
                        );
                ...
                if (directionY === 'up') knife.body.velocity.y = -500;
                else if (directionY === 'down') knife.body.velocity.y = 500;
                if (directionX === 'left') knife.body.velocity.x = -500;
                else if (directionX === 'right') knife.body.velocity.x = 500;
                if (directionX === 'none' && directionY === 'none') {
                    if (lastX === 'left') knife.body.velocity.x = -500;
                    if (lastX === 'right') knife.body.velocity.x = 500;
                    if (lastY === 'up') knife.body.velocity.y = -500;
                    if (lastY === 'down') knife.body.velocity.y = 500;
                }
                if (directionX !== 'none' || directionY !== 'none') {
                    lastX = directionX;
                    lastY = directionY;
                ...
                else knife.body.angularVelocity = -500;
            ...
            if (time >= eggTimer && eggFlag) {
                eggTimer = time + 10000;
                for (let i = 0; i < phase * 10; i++) {
                    game.time.events.add(
                        200 * i,
                        () => eggPop(Chicken.x, Chicken.y, i, phase),
                        this,
                    );
                }
                eggFlag = false;
            ...
            for (let i = 0; i < eggs.length; i++) {
                ...
                if (eggCollided) // you got hit!
                {
                    ...
                    if (randy === 1) {
                        // you have salmonella!
                        salmonellaFlag = true;
                        const salm = game.add.sprite(300, -5, 'salm');
                        salm.scale.x = 0.04;
                        salm.scale.y = 0.04;
                    }
                    const splat = game.add.sprite(cur.x, cur.y, 'eggSplat');
                    game.time.events.add(1500, () => eggDestroy(splat), this);
                    switch (phase) {
                        case 1:
                            splat.scale.x = 1.0;
                            splat.scale.y = 1.0;
            ...
            if (Chicken !== null && Chicken.body !== null) {
                Chicken.body.velocity.y = Math.sin(0.0005 * time) * eSpeed * 2;
